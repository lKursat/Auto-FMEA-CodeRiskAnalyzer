...
import { AnalysisResult, ClassInfo } from './types';
...
function getLanguage(doc: vscode.TextDocument): string | null {
...
const RISK_TO_SEVERITY: Record<string, vscode.DiagnosticSeverity> = {
...
function updateDiagnostics(doc: vscode.TextDocument, result: AnalysisResult): void {
...
function updateStatusBar(result: AnalysisResult): void {
...
async function analyzeDocument(doc: vscode.TextDocument): Promise<void> {
    ...
    if (docWorkspaceFolder) {
        ...
        outputChannel.appendLine(
            `[Auto-FMEA] WARNING: File "${doc.uri.fsPath}" is outside the workspace. ` +
            `Using workspace root "${projectRoot}" for dependency scanning.`
        );
    } else {
        projectRoot = path.dirname(doc.uri.fsPath);
        outputChannel.appendLine(
            `[Auto-FMEA] WARNING: No workspace folder open. ` +
    ...
    outputChannel.appendLine(
        `[Auto-FMEA] project-root resolved to: "${projectRoot}" ` +
        `(file: "${doc.uri.fsPath}")`
    );
...
export function activate(context: vscode.ExtensionContext): void {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('autofmea.showDashboard', () => {
            DashboardPanel.createOrShow(context.extensionUri);
            // Send cached data if available
            const activeDoc = vscode.window.activeTextEditor?.document;
            if (activeDoc) {
                const cached = analysisCache.get(activeDoc.uri.toString());
                if (cached) { DashboardPanel.update(cached); }
            }
        }),
    ...
    context.subscriptions.push(
        vscode.workspace.onDidSaveTextDocument((doc) => {
            const config = vscode.workspace.getConfiguration('autofmea');
            if (!config.get<boolean>('analyzeOnSave', true)) { return; }

            if (!getLanguage(doc)) { return; }
    ...
    context.subscriptions.push(
        vscode.window.onDidChangeActiveTextEditor((editor) => {
            ...
            if (cached) { updateStatusBar(cached); }
        }),
    );
